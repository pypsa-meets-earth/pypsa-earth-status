...
generated by the workflow in agreement to the specifications in the config file
and creates a PyPSA network whose statistics match the reference data.
...
- Generators matching the reference installed capacity by carrier by
  setting ``p_nom`` to the installed capacity (MW)

This script can be run using the rule ``build_reference_pypsa_network``
